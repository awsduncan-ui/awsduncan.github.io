...
function nationalPage(regionPubs, regionCounts, manifest) {
  ...
  const cards = selected
    ...
    .join("\n");
  const content = `<section class="section directory-intro"><div class="shell directory-intro-grid"><div><h2>Start with a regional guide</h2><p class="body-copy">Each guide is built from checked public listings and shows the full address, recorded play type, practical facilities and last verification date. The first wave focuses on areas where the directory already has useful depth.</p></div><form class="postcode-finder" data-postcode-finder><label for="postcode-finder">Find the nearest first-wave guide</label><div class="directory-filter-row"><input id="postcode-finder" name="location" type="search" autocomplete="postal-code" placeholder="Postcode or town"><button class="btn btn-coral" type="submit">Find a guide</button></div><p data-postcode-status aria-live="polite">Enter a UK postcode or one of the places covered below.</p></form></div></section>
...
function homepageGateway(regionPubs, regionCounts) {
  ...
  return `<!-- DIRECTORY_GATEWAY_START -->
    <section class="section directory-gateway" id="directory" aria-labelledby="directory-title">
      <div class="shell">
        <div class="directory-intro-grid">
          <div><p class="eyebrow">Browse on the web</p><h2 id="directory-title">Find pubs with playgrounds near you</h2><p class="body-copy">Start with checked pub listings, addresses and family facilities—no app installation required. Choose a regional guide or search by postcode.</p></div>
          <form class="postcode-finder" data-postcode-finder><label for="homepage-postcode">Find a regional guide</label><div class="directory-filter-row"><input id="homepage-postcode" name="location" type="search" autocomplete="postal-code" placeholder="Postcode or town"><button class="btn btn-coral" type="submit">Find pubs</button></div><p data-postcode-status aria-live="polite">We’ll route you to the closest first-wave guide.</p></form>
        </div>
